        ...
        <tbody>
          @if(isset($countries) && count($countries) > 0)
            @foreach($countries as $country)
              @php
                $isAntarctica = strtolower($country->name) === 'antarctica';
                ...
                if ($isAntarctica) {
                  ...
                  $gdpStr = ($gdp !== null && $gdp > 0) ? number_format($gdp/1e9, 1) : 'N/A';
                  $popStr = ($population !== null && $population > 0) ? number_format($population/1e6, 2).' M' : '0.00 M';
                  $inflStr = ($inflation !== null) ? number_format($inflation, 2).'%' : '0.00%';
                  $riskScoreStr = ($riskScore !== null) ? number_format($riskScore, 1) : '0.0';
                }
              @endphp
              <tr onclick="viewCountry({{ $country->id }})" style="cursor:pointer;">
                ...
                <td class="col-align-right {{ ($inflStr !== 'N/A' && $inflStr !== '0.00%' && (float)$inflStr > 5) ? 'text-danger' : '' }}">{{ $inflStr }}</td>
...
function initCountriesTable() {
  ...
  try {
    tbody.innerHTML = STATE.countries.map(c => {
      ...
      const isAntarctica = name.toLowerCase() === 'antarctica';
      
      let region, currency, gdpStr, popStr, inflStr, riskScoreStr, riskLevel;
      ...
      if (isAntarctica) {
        ...
        const gdpVal = parseFloat(c.gdp);
        gdpStr = (isNaN(gdpVal) || gdpVal <= 0) ? 'N/A' : (gdpVal / 1e9).toFixed(1);
        
        const popVal = parseFloat(c.population);
        popStr = (isNaN(popVal) || popVal <= 0) ? '0.00 M' : (popVal / 1e6).toFixed(2) + ' M';
        
        const inflVal = parseFloat(c.inflation);
        inflStr = isNaN(inflVal) ? '0.00%' : inflVal.toFixed(2) + '%';
        
        const riskScoreVal = parseFloat(c.risk_score);
        riskScoreStr = isNaN(riskScoreVal) ? '0.0' : riskScoreVal.toFixed(1);
      ...
      const inflClass = inflStr !== 'N/A' && inflStr !== '0.00%' && parseFloat(inflStr) > 5 ? 'text-danger' : '';
      ...
      return \`<tr onclick="viewCountry(\${c.id})" style="cursor:pointer;">
        ...
        <td class="col-align-left" style="color:var(--text-secondary);">\handleTabSelect\${region}</td>
        <td class="col-align-left">\${currency}</td>
        <td class="col-align-right">\${gdpStr}</td>
        <td class="col-align-right">\${popStr}</td>
        <td class="col-align-right \text-right \${inflClass}">\${inflStr}</td>
        <td class="col-align-center"><span style="font-family:var(--font-mono);font-weight:700;">\${riskScoreStr}</span></td>
...
console.log("Saved dashboard.blade.php with Antarctica fallback redesign!");
